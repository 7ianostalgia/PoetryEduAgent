...
PoetryEduAgent 使用两份职责不同的 SQLite 数据库：一份保存诗词知识，一份保存实际运行产生的任务与学习记录。数据库文件只保存在本地或服务器，不提交到公开仓库。
...
默认知识库路径为 `data/poetry_edu.db`，可通过初始化脚本在本地生成。运行数据库可放在任意持久化目录，由服务启动配置决定。
